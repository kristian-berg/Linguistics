# Monte Carlo simulation for the German word formation pattern -isch
# c2018 Kristian Berg & Yannick Habecker
...
### set parameters
NUM_PROCESSES = 1
nr_sim = 1000                        # nr of simulations
decs = range(1490,1910,10)        # decades in the corpus
decs_result = range(1700,1910,10) # decades of interest
target_size = 10000000             # maximum corpus size per decade
...
def shuffle_dec(decade, n):
    ...
    if real_size <= target_size:
        return (texts_dta.loc[texts_dta.Dekade == decade], None, None)
    else:
        counter = 0
        ...
        last_text_limit = counter - target_size if counter > target_size else None
        # if file_limit is not None:
        #     df = pd.concat([texts_dec.iloc[0:j-1,:], texts_dec.iloc[j-1,:file_limit]], axis=0)
        #     print(df[:,2].sum())
        #     return df
        return (texts_dec.iloc[0:j-1,:], texts_dec.iloc[j-2,0], last_text_limit)
...
def get_suffix_sub(dec, n):
    ...
    (sub_corpus, last_text, last_text_limit) = shuffle_dec(dec, n) # pd.concat([shuffle_dec(dec, n) for dec in decs], axis=0)
    ...
    if last_text_limit is not None:
        suffix_sub = suffix_sub.query('not(Datei == \'%s\' and RowNum >= %d)' % (last_text, last_text_limit))
        assert(suffix_sub.query('(Datei == \'%s\' and RowNum >= %d)' % (last_text, last_text_limit)).empty)
...
def do_result(result):
    ...
    for col, value in new_types_suffix["all_tokens"].iteritems():
        tokens_global.set_value(n, col, value)
    for col, value in new_types_suffix["hapax"].iteritems():
        hapax_global.set_value(n, col, value)
    for col, value in new_types_suffix["hapax_new"].iteritems():
        hapax_and_new_global.set_value(n, col, value)
    for col, value in new_types_suffix["min"].iteritems():
        vneo_global.set_value(n, col, value)
    for col, value in new_types_suffix["all_types"].iteritems():
        types_global.set_value(n, col, value)
...
if __name__ == "__main__":
    ...
    suffix_raw  = pd.read_csv('isch.csv', sep = ",")       # this file contains a line for each -nis attestation in the corpus, together with the respective decade and filename
    ...
    # save dataframe
    pneo_global.to_csv("ISCH_pneo_global_100000.csv", encoding = "utf-8")
    tokens_global.to_csv("ISCH_tokens_global_100000.csv", encoding = "utf-8")
    hapax_global.to_csv("ISCH_hapax_global_100000.csv", encoding = "utf-8")
    hapax_and_new_global.to_csv("ISCH_hapax_and_new_global_100000.csv", encoding = "utf-8")
    vneo_global.to_csv("ISCH_vneo_global_100000.csv", encoding = "utf-8")
    types_global.to_csv("ISCH_types_global_100000.csv", encoding = "utf-8")
