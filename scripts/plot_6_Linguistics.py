...
pneo_tum = pd.read_csv("TUM_pneo_global_100000.csv", sep = ",", index_col = 0)
pneo_bar = pd.read_csv("BAR_pneo_global_100000.csv", sep = ",", index_col = 0)
pneo_ieren = pd.read_csv("IEREN_pneo_global_100000.csv", sep = ",", index_col = 0)
pneo_isch = pd.read_csv("ISCH_pneo_global_100000.csv", sep = ",", index_col = 0)
pneo_nis = pd.read_csv("NIS_pneo_global_100000.csv", sep = ",", index_col = 0)
...
ax1.plot(list(pneo_tum),pneo_tum.mean(axis = 0, skipna =  True).values, linewidth = 0.7, color = "black")
conf_int(pneo_tum, decs, 0.99, 'grey', ax1)
conf_int(pneo_tum, decs, 0.95,'dimgrey', ax1)
figure_env(ax1, "-tum")

ax2 = fig.add_subplot(322)
ax2.plot(list(pneo_bar),pneo_bar.mean(axis = 0, skipna =  True).values, linewidth = 0.7, color = "black")
conf_int(pneo_bar, decs, 0.99, 'grey', ax2)
conf_int(pneo_bar, decs, 0.95,'dimgrey', ax2)
figure_env(ax2, "-bar")

ax3 = fig.add_subplot(323)
ax3.plot(list(pneo_ieren),pneo_ieren.mean(axis = 0, skipna =  True).values, linewidth = 0.7, color = "black")
conf_int(pneo_ieren, decs, 0.99, 'grey', ax3)
conf_int(pneo_ieren, decs, 0.95,'dimgrey', ax3)
figure_env(ax3, "-ieren")

ax4 = fig.add_subplot(324)
ax4.plot(list(pneo_isch),pneo_isch.mean(axis = 0, skipna =  True).values, linewidth = 0.7, color = "black")
conf_int(pneo_isch, decs, 0.99, 'grey', ax4)
conf_int(pneo_isch, decs, 0.95,'dimgrey', ax4)
figure_env(ax4, "-isch")

ax5 = fig.add_subplot(325)
ax5.plot(list(pneo_nis),pneo_nis.mean(axis = 0, skipna =  True).values, linewidth = 0.7, color = "black")
conf_int(pneo_nis, decs, 0.99, 'grey', ax5)
conf_int(pneo_nis, decs, 0.95,'dimgrey', ax5)
figure_env(ax5, "-nis")

#fig.text(0.5, 0.005, 'decades', ha='center', size = 14)
fig.text(0.005, 0.5, '#new types/#all types', va='center', rotation='vertical', size = 14)

fig.tight_layout(pad = 2)
fig.savefig('plot_6_Linguistics.png', dpi=800)
